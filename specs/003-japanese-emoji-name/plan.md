...
**変更規模**: `src/utils/normalize.ts` の1行（sanitize regex）の修正と、正規化コメントの更新のみ。
...
**構造の決定**: 変更は `src/utils/normalize.ts` の1ファイルのみ。新規ファイルの追加なし。既存のモジュール構造をそのまま維持する。

## 変更詳細

### 改修A: sanitize regex の拡張
