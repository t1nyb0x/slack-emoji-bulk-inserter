...
- Q: 1回のドロップに同名ファイルが含まれている場合（例: フォルダAとBの smile.png を同時にドロップ）、どう処理するか？ → A: バッチ内も重複検知する。`filesToRegistrationItems` 内で作成中の items 配列と `registrationQueue` の両方をチェックし、2つ目以降を Skipped にする
...
- **同一ドロップ内に同名ファイルが含まれる場合**: `filesToRegistrationItems` 内で作成中の items 配列も重複チェック対象とし、同一バッチ内の2つ目以降を Skipped にする
